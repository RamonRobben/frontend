...
aurousScript(function() {
  ...
  aurousScript.player.onloadeddata = function(e) {};
  ...
  aurousScript.player.playNextCollection = function() {
    var nextRow = aurousScript(window.previousCollectionRow).closest('tr').next('tr');
  ...
  aurousScript.player.playNextPlaylist = function() {
    ...
    var nextRow = aurousScript(window.previousPlaylist).closest('tr').next('tr');
    window.previousPlaylist.removeClass('result-now-playing');
    aurousScript('#playlist-row-icon-' + window.previousPlaylistId).html('play_arrow');
    var parent = nextRow.closest('tr');
  ...
  aurousScript.player.playNextSearchResult = function() {
    var nextRow = aurousScript(window.previousSearchResult).closest('tr').next('tr');
  ...
  aurousScript.player.playLastSearchResult = function() {
    var nextRow = aurousScript(window.previousSearchResult).closest('tr').prev('tr');
    window.previousSearchResult.removeClass('result-now-playing');
    aurousScript('#row-icon-' + window.previousSearchId).html('play_arrow');
    var parent = nextRow.closest('tr');
    var id = parent.attr('data-id');
  ...
  aurousScript.player.playLastArtist = function() {
    var lastRow = aurousScript(window.previousCollectionRow).closest('tr').prev('tr');
    window.previousCollectionRow.removeClass('result-now-playing');
    aurousScript('#artist-row-icon-' + window.previousCollectId).html('play_arrow');
  ...
  aurousScript.player.playLastPlaylist = function() {
    ...
    var lastRow = aurousScript(window.previousPlaylist).closest('tr').prev('tr');
    window.previousPlaylist.removeClass('result-now-playing');
    aurousScript('#playlist-row-icon-' + window.previousPlaylistId).html('play_arrow');
    var parent = lastRow.closest('tr');
    var id = parent.attr('playlist-data-id');
  ...
  aurousScript.player.toggleMuted = function() {
    // set muted.
    aurousScript.player.muted = !aurousScript.player.muted;
    // return current Muted Boolean
    return aurousScript.player.muted;
  };

  aurousScript.player.toggleAutoPlay = function() {
    aurousScript.player.autoplay = !aurousScript.player.autoplay;
  };
  ...
  aurousScript.player.alterPlayback = function() {
    aurousScript.player[aurousScript.player.paused ? 'play' : 'pause']();
    aurousScript.showControl(aurousScript.player.paused ? 'Play' : 'Pause');
  };
  ...
  // Fix CORS when use createMediaElementSource
  // see https://code.google.com/p/chromium/issues/detail?id=477364
  aurousScript.player.crossOrigin = 'anonymous';
  // Fix up prefixing
  window.AudioContext = window.AudioContext || window.webkitAudioContext;
  aurousScript.player.context = new AudioContext();
